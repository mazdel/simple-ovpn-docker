...
# TODO : if user provide a password as argument, then
# /opt/easyrsa/easyrsa --passout=pass:${CLIENT_PASS} --batch build-client-full "$CLIENT_NAME"
# /usr/bin/openssl rsa -in "$OVPN_CLIENT_DIR/keys/$CLIENT_NAME.key" -text <(echo -e ${CLIENT_PASS}) > "$OVPN_CLIENT_DIR/keys/$CLIENT_NAME.key.pem"
# or else...
